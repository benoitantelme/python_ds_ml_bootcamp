...
# 
# Just go along with this notebook and try to complete the instructions or answer the questions in bold using your
...
# In[136]:
print('Titles: ' + str(len(df['title'].unique())))

# ## Creating new features

# ** In the titles column there are "Reasons/Departments" specified before the title code. These are EMS, Fire, and
# Traffic. Use .apply() with a custom lambda expression to create a new column called "Reason" that contains this
# string value.**
# 
...
# are actually DateTime objects, use .apply() to create 3 new columns called Hour, Month, and Day of Week. You will
# create these columns based off of the timeStamp column, reference the solutions if you get stuck on this step.**
...
# ** Notice how the Day of Week is an integer 0-6. Use the .map() with this dictionary to map the actual string names
...
# ** Now see if you can use seaborn's lmplot() to create a linear fit on the number of calls per month. Keep in mind
...
# **Create a new column called 'Date' that contains the date from the timeStamp column. You'll need to use apply
...
# so that the columns become the Hours and the Index becomes the Day of the Week. There are lots of ways to do this,
...
# In[203]:
byDowHour = df.groupby(['Day of Week', 'Hour']).count()['Reason'].unstack()
print(byDowHour.head())
# ** Now create a HeatMap using this new DataFrame. **

# In[204]:
sns.heatmap(byDowHour, cmap='coolwarm')
plt.show()

# ** Now create a clustermap using this DataFrame. **

# In[205]:
sns.clustermap(byDowHour,cmap='coolwarm')
plt.show()

# ** Now repeat these same plots and operations, for a DataFrame that shows the Month as the column. **

# In[207]:
byDowMonth = df.groupby(['Day of Week', 'Month']).count()['Reason'].unstack()
print(byDowMonth.head())

# In[208]:
sns.heatmap(byDowMonth, cmap='coolwarm')
plt.show()

# In[209]:
sns.clustermap(byDowMonth, cmap='coolwarm')
plt.show()
